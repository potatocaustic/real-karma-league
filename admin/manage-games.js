    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            if (userDoc.exists() && userDoc.data().role === 'admin') {
                ...
                // ... (logout listener and initialize page)
                initializePage();
            } else {
...
async function fetchAndDisplayGames(seasonId, week) {
    ...
    // --- MODIFIED: Query the new /seasons/{id}/games subcollection ---
    const gamesCollectionRef = collection(db, `seasons/${seasonId}/games`);
    const gamesQuery = query(gamesCollectionRef, where("week", "==", week));
...
gamesListContainer.addEventListener('click', async (e) => {
    if (e.target.matches('.btn-admin-edit')) {
        ...
        // --- MODIFIED: Get doc from the correct subcollection ---
...
scoreForm.addEventListener('submit', async (e) => {
    ...
    // --- MODIFIED: Update doc in the correct subcollection ---
    const gameRef = doc(db, `seasons/${currentSeasonId}/games`, gameId);

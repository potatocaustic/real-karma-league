...
function getPostseasonGameLabel(seriesName) {
    ...
    const seriesTypeMap = {
        ...
        'WCF': `WCF ${gameNumberString}`, 'ECF': `ECF ${gameNumberString}`, 'Finals': `RKL Finals ${gameNumberString}`,
    };
...
async function fetchInitialPageData(seasonId) {
    ...
    const teamPromises = teamsSnapshot.docs.map(async (teamDoc) => {
        let teamData = { id: teamDoc.id, ...teamDoc.data(), wins: 0, losses: 0 };
        ...
        if (seasonalRecordSnap.exists()) {
            teamData = { ...teamData, ...seasonalRecordSnap.data() };
        }
        return teamData;
    });
...
function setupWeekSelector() {
    ...
    weekButtonsContainer.innerHTML = allKnownWeeks.map(week => {
...
async function displayWeek(week) {
    ...
    gamesContent.innerHTML = sortedDates.map(date => {
        const dateGamesHTML = gamesByDate[date].map(game => {
            ...
            const statusHTML = finalStatusText;
    ...
    document.querySelectorAll('.game-card.completed').forEach(card => {
...
async function showGameDetails(gameId, isLive, gameDate = null) {
    ...
    try {
        ...
        contentArea.innerHTML = `<div class="game-details-grid">${generateLineupTable(team1Lineups, modalTeam1, winnerId === team1.id, false)}${generateLineupTable(team2Lineups, modalTeam2, winnerId === team2.id, false)}</div>`;

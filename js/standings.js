...
async function fetchAllTeamsAndRecords() {
    ...
    recordsSnapshot.forEach(doc => {
        // Server-side filtered by seasonId - all results match activeSeasonId
...
function renderPowerRankings(version) {
    ...
    const label = versionNumber === 0 ? 'v0 (Preseason)' : (league === 'minor' ? `v${versionNumber}` : `v${versionNumber} (Week ${versionNumber * 3})`);
